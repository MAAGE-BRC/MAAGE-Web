define([
    ...
    return declare([WidgetBase, Templated, _WidgetsInTemplateMixin], {
        ...
        createCharts: function() {
            ...
            // Helper function now accepts a theme name
            const createChart = (widgetClass, node, query, theme) => {
                if (!node) return;
                const chart = new widgetClass({ title: '', theme: theme || 'maage-echarts-theme' });
                chart.placeAt(node);
                chart.startup();
                chart.showLoading();
            ...
            // Instantiate charts, passing in the desired theme for each one.
            createChart(Doughnut, this.statusChartNode, `${baseQuery}&facet((field,genome_status),(mincount,1))&limit(0)`, 'maage-muted');
            createChart(Doughnut, this.countryChartNode, `${baseQuery}&facet((field,isolation_country),(mincount,1),(limit,10))&limit(0)`, 'maage-muted');
            createChart(VerticalBar, this.hostChartNode, `${baseQuery}&facet((field,host_common_name),(mincount,1),(limit,10))&limit(0)`, 'maage-muted');
            createChart(Doughnut, this.sourceChartNode, `${baseQuery}&facet((field,isolation_source),(mincount,1),(limit,10))&limit(0)`, 'maage-muted');
            createChart(Doughnut, this.serotypeChartNode, `${baseQuery}&facet((field,serovar),(mincount,1),(limit,10))&limit(0)`, 'maage-muted');
            createPivotChart(StackedBar, this.serotypeOverTimeChartNode, `${baseQuery}&facet((pivot,(collection_year,serovar)),(mincount,1))&limit(0)`, 'maage-muted');
        },
